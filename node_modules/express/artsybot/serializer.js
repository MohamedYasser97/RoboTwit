...
if(fs.existsSync('./artsybot/data/data.json')==false)
	fs.appendFileSync('./artsybot/data/data.json','{}');

var map = JSON.parse(fs.readFileSync('./artsybot/data/data.json', 'utf8'));
...
module.exports={
	...
	*/
	add : function(url){
		...
		let stat = fs.statSync('./artsybot/data/data.json');

		fs.truncateSync('./artsybot/data/data.json',stat.size-1);
		fs.appendFileSync('./artsybot/data/data.json',`${stat.size==2 ? '' : ','}"${this.generateKey(url)}":1}`);
