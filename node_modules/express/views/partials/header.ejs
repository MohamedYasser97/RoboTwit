<nav class="navbar navbar-expand-lg navbar-dark bg-dark">

 <% if(currentUser) {%>
  <li class="nav-item dropdown" id="displaypicture">
        <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
          <img <% if(currentUser.image !=''){%>src="<%=currentUser.image%>"<%}else{%> src="https://static.getjar.com/icon-50x50/1f/826197_thm.gif"<%}%> height="50" width="50">
        </a>
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
          <form action = "/settings" method = "GET">
          <button type = "submit" class="dropdown-item">Settings</button>
          </form>
          <div class="dropdown-divider"></div>
          <form action = "/logout" method = "GET">
          <button type = "submit" class="dropdown-item">Log out</button>
        </form>
        </div>
      </li>

  <% } %>

    <a class="navbar-brand" href="#">
      <img src="/docs/4.1/assets/brand/bootstrap-solid.svg" width="30" height="30" class="d-inline-block align-top" alt="">
      RoboTwit
    </a>
    <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation">
      <span class="navbar-toggler-icon"></span>
    </button>

    <div class="collapse navbar-collapse" id="navbarSupportedContent">
      <ul class="navbar-nav mr-auto">
        <li class="nav-item ">

            <form action="/home" method="GET">
          <button id="Navbarbutton" type="submit" class="nav-link" href="#">Home</button>
        </form>
        </li>
        <li class="nav-item ">
          <form action="/about" method="GET">
          <button id="Navbarbutton" type="submit" class="nav-link" href="#">About</button>
        </form>
        </li>
        </li>
        <li class="nav-item active">
          <form action="/contact" method="GET">
          <button id="Navbarbutton" type="submit" class="nav-link" href="#">Contact</button>
        </form>
        </li>
      </ul>

      <% if(currentUser){ %>
        <!-- if bot is running change button to stop TO DO-->
        <% if(currentUser.accType=="developer"){%>
          <button type="button" class="btn btn-success">Start Bot</button> &nbsp; &nbsp;
      <% } %>
      <li class="nav-item dropdown navbar-right dropleft">
          <p>Signed in as: <%= currentUser.username %></p>
      </li>
      <% } %>

    </div>
  </nav>

  <div class="container">
       <% if(error && error.length > 0) { %>
         <div class="alert alert-danger"><p><%=error %></p></div>
       <% } %>

       <% if(success && success.length > 0) { %>
         <div class="alert alert-success"><p><%= success %></p></div>
       <% } %>
    </div>
